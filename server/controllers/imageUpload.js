...
exports.uploadPicture = asyncHandler(async (req, res, next) => {
  ...
  let imageUrlPath;
    try {
      imageUrlPath = await cloudinary.uploader.upload(uploadedPhoto, async (err) => {
        if (err) {
          console.error(err);
        }
      });
    ...
    const profile = await Profile.findOneAndUpdate(userId, { photo: url}, {new: true});
...
exports.removePicture = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const profile = await Profile.findOneAndUpdate(userId, {photo: ""}, {new : true});
